...
const financeNav = [
  ...
  { label: 'Revenue Store', href: '/dashboard/finance/revenue-store', icon: TrendingUp },
    ...
    <aside className="bg-white/80 backdrop-blur-xl border-r border-gray-200/50 flex flex-col h-full w-full">
      ...
      <nav className="flex-1 px-3 py-2 flex flex-col gap-1 overflow-y-auto">
        {financeNav.map(item => {
          ...
          const active = pathname === item.href || (pathname.startsWith(item.href + '/') && !financeNav.some(n => n.href !== item.href && pathname.startsWith(n.href) && n.href.length > item.href.length))
