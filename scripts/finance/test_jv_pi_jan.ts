  ...
  try {
    ...
    const res = await axios.get(`${apiBaseUrl}/api/journal-voucher/list.do`, {
      ...
      paramsSerializer: params => {
         ...
         searchParams.append('filter.sourceType.op', 'EQUAL');
         searchParams.append('filter.sourceType.val', 'PURCHASE_INVOICE');
    ...
    const d = res.data.d;
    console.log(`Found ${d.length} Purchase Invoice journals in Jan 2025.`);
    if (d.length > 0) {
